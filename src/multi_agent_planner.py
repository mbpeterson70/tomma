...
class MultiAgentPlanner():
    ...
    def solve_min_time(self, x0, xf, u0=None, uf=None, x_bounds=None, u_bounds=None, u_diff_bounds=None):
        ...
        '''
        self.opti = Opti()
        x = [self.opti.variable(self.dynamics.x_shape, self.N+1) for m in range(self.M)]
        u = [self.opti.variable(self.dynamics.u_shape, self.N) for m in range(self.M)]
        tf = self.opti.variable()
        # for m in range(self.M):
        self.opti.set_initial(tf, 10.0)

        self.opti.minimize(tf)
        
        dt = tf/self.N
        for k in range(self.N):
            for m in range(self.M):
                self.opti.subject_to(x[m][:,k+1] == self.dynamics.propagate(x[m][:,k], u[m][:,k],  dt))
        
        for i in range(self.dynamics.x_shape):
            for m in range(self.M):
                self.opti.subject_to(x[m][i,0] == x0[m,i])
                self.opti.subject_to(x[m][i,-1] == xf[m,i])

        self.opti.subject_to(tf > 1.)

        if u0 is not None:
            for i in range(self.dynamics.u_shape):
                for m in range(self.M):
                    self.opti.subject_to(u[m][i,0] == u0.item(i))
        if uf is not None:
            for i in range(self.dynamics.u_shape):
                for m in range(self.M):
                    self.opti.subject_to(u[m][i,-1] == uf.item(i))
        ...
        if x_bounds is not None:
            for i in range(self.dynamics.x_shape):
                if np.isinf(x_bounds[i,0]) and np.isinf(x_bounds[i,1]):
                    continue
                for m in range(self.M):
                    self.opti.subject_to(self.opti.bounded(x_bounds[i,0], x[m][i,:], x_bounds[i,1]))
        
        if u_bounds is not None:
            for i in range(self.dynamics.u_shape):
                for m in range(self.M):
                    self.opti.subject_to(self.opti.bounded(u_bounds[i,0], u[m][i,:], u_bounds[i,1]))
            ...
            for i in range(self.dynamics.u_shape):
                ...
                for m in range(self.M):
                    for k in range(self.N - 1):
                        self.opti.subject_to(((u[m][i,k] - u[m][i,k+1])/dt)**2 <= (u_diff_bounds[i])**2)
        ...
        for ob in self.obstacles:
            ...
            # [(x[j,:] - ob['position'][i])**2 for i, j in enumerate(self.dynamics.physical_state_idx)]
                self.opti.subject_to(sum([(x[m][j,:] - ob['position'][i])**2 for i, j in enumerate(self.dynamics.physical_state_idx)]) >= ob['radius']**2)

        for m1 in range(self.M):
            for m2 in range(self.M):
                if m1 == m2:
                    continue
                self.opti.subject_to((x[m1][0,:] - x[m2][0,:])**2+(x[m1][1,:] - x[m2][1,:])**2 >= self.min_allowable_dist)

        self.opti.solver('ipopt', self.solver_opts)
        self.x, self.u, self.tf = x, u, tf
